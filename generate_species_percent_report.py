...
def render_graph_filters_section(min_year: int, max_year: int) -> str:
    ...
    parts.append("    <label class=\"ranges-toggle\"><input id=\"show-ranges\" type=\"checkbox\" > Show species ranges</label>")
    ...
    parts.append("          <label class=\"group-option\"><input type=\"checkbox\" name=\"group-filter\" value=\"fungi\" > Fungi</label>")
    parts.append("          <label class=\"group-option\"><input type=\"checkbox\" name=\"group-filter\" value=\"lichen\" checked> Lichen</label>")
    parts.append("          <label class=\"group-option\"><input type=\"checkbox\" name=\"group-filter\" value=\"slime_molds\" > Slime Molds</label>")
    parts.append("          <label class=\"group-option\"><input type=\"checkbox\" name=\"group-filter\" value=\"mosses\" > Mosses</label>")
    parts.append("        </div>")
    ...
    return "\n".join(
        [
            ...
            *sections,
            ...
            "          }",
            "          shown += 1;",
            "          const markerColor = groupColorMap[point.group] || '#0b7285';",
            "",
            ...
            "            : '';",
            "          const popupHtml = '<strong>' + species + '</strong><br>Group: ' + groupLabel + '<br>Year: ' + point.year + '<br>Observed on: ' + observedOn + details;",
            "          hitMarker.bindPopup(popupHtml);",
            "          marker.addTo(markerLayer);",
            "          hitMarker.addTo(markerLayer);",
            "        });",
            "",
